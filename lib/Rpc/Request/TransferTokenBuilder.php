...
class TransferTokenBuilder
{
    ...
    /**
     * Builds a token payload, with uploading blobs or attachments.
     *
     * @return TokenPayload the token payload
     * @throws IllegalArgumentException
     */
    public function buildWithBlobAttachments()
    {
        ...
        $redeemer = $this->payload->getTransfer()->getRedeemer();
        if ($redeemer->getId() == null && $redeemer->getAlias() == null) {
            throw new IllegalArgumentException("No redeemer on token");
        }
        ...
        if (!empty($attachmentUploads)) {
            $attachmentsRepeatedField = $this->payload->getTransfer()->getAttachments();

            $attachments = array();
            foreach ($attachmentsRepeatedField as $attachment) {
                $attachments[] = $attachment;
            }

            $attachments = array_merge($attachments, $attachmentUploads);
            $this->payload->getTransfer()->setAttachments($attachments);
        }
